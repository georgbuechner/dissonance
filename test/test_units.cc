...
TEST_CASE("calling getting way has target included", "[units]") {
  ...
  auto way_to_epsp_target = neurons[pos]->GetWayWithTargetIncluded(UnitsTech::EPSP);
  REQUIRE(way_to_epsp_target.size() == 1);
  REQUIRE(way_to_epsp_target.front() == epsp_target);
  auto way_to_ipsp_target = neurons[pos]->GetWayWithTargetIncluded(UnitsTech::IPSP);
