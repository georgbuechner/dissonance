  ...
  while (true) {
    ...
    // T: Technology
      ...
      for (const auto& it : player_one_->technologies()) {
        ...
        mapping[it.first-UnitsTech::WAY] = {units_tech_name_mapping.at(it.first) 
      ...
      if (player_one_->AddTechnology(technology))
        PrintMessage("selected: " + units_tech_name_mapping.at(technology), false);
...
position_t Game::SelectNeuron(Player* p, int type) {
  std::string msg = "Choose " + units_tech_name_mapping.at(type) + ": ";
