...
void Field::add_ki() {
  ...
  ki_[std::make_pair(y, x)] = DEN;
  field_[y][x] = ki_[std::make_pair(y,x)];
...
void Field::add_player_soldier() {
  auto pos = find_free(player_den_.first, player_den_.second, 1);
  field_[pos.first][pos.second] = '1';
}
...
void Field::print_field() {
  for (int l=0; l<lines_; l++) {
    for (int c=0; c<cols_; c++) {
      ...
      mvaddch(10+l, 10+2*c, field_[l][c]);
